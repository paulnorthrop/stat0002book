...
The **complement** of an event $A$ is the event that $A$ does not occur.  This can be denoted $\mbox{not}A$ or $\bar{A}$ or $A^c$. We say that the events $A$ and $\mbox{not}A$ are **complementary**. The events $A$ and $\mbox{not}A$ are mutually exclusive and $S=\{A, \mbox{not}A\}$.  We have already seen that $P(S) = 1$. Therefore,
