...
If two events $A$ and $B$ are mutually exclusive then $P(A \text{ and } B)=0$. Substituting this into equation \@ref(eq:ordef) we find that, **if events $A$ and $B$ are mutually exclusive**
\begin{equation}
...
The **complement** of an event $A$ is the event that $A$ does not occur.  This can be denoted $\text{not}A$ or $\bar{A}$ or $A^c$. We say that the events $A$ and $\text{not}A$ are **complementary**. The events $A$ and $\text{not}A$ are mutually exclusive and $S=\{A, \text{not}A\}$.  We have already seen that $P(S) = 1$. Therefore,
