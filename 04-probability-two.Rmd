...
* $P(M \text{ or } F) = 1$, that is, together $M$ and $F$ cover the entire sample space of the possible values of sex.  This means that $M$ and $F$ are **exhaustive** events.  More generally, a set of exhaustive events has the property that at least one of these events must occur.
...
For example, $P(A \mid B , C)$, or $P(A \mid B \text{ and } C)$, or $P(A \mid B \cap C)$ is the conditional probability that event $A$ occurs given that **both** events $B$ and $C$ have occurred.  The general principle is that we have conditioned on all events that are placed on the right hand side of the conditional $\mid$ symbol. All the results that we have seen can be extended to probabilities conditioned on more than one event.  
