...
Figure \@ref(fig:lminfluence) shows some examples where there are one or two outlying observations. The greater the influence of the outlier(s) the greater the change in the fitted regression line when the they are removed. Generally speaking, an observation with an outlying (small or large) value of $x$ has a greater potential (the technical term is **leverage**) to affect the fitted regression line than an observation which has an average $x$ value.  This can be seen by comparing plots (a) and (b).  
