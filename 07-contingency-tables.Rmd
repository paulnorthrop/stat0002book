...
\[ \widehat{P}(M)=\frac{2691}{4526}, \,\, \widehat{P}(F)=\frac{1835}{4526}, \,\,
\widehat{P}(A)=\frac{1755}{4526}, \,\, \widehat{P}(R)=\frac{2771}{4526}.  \]
...
If $X$ and $Y$ are independent, and if the expected frequencies are large, each Pearson residual, and each standardised Pearson residual, has approximately a normal distribution with mean 0. In addition the standardised Pearson residuals have an approximate variance of 1. So, **if these assumptions are true**, the standardised Pearson residuals are approximately N(0,1).  Owing to the properties of the N(0,1) distribution, most (approximately 95\%) of the residuals should lie in the range $(-2, 2)$.  The presence of residuals that have a large magnitude would be surprising and may suggest that $X$ and $Y$ are not independent.

The residuals, Pearson residuals and standarised Pearson residuals for the Berkeley admissions example are given in Figure \@ref(fig:berkres)
...
frequencies are sufficiently large) then, before we have observed the data, the 
...
```{r echo=FALSE, odds, fig.show='hold', fig.cap='The value of the odds of an event for different probabilities of that event.', out.width='60%'}
...
If $P(A~|~M)$ and $P(A~|~F)$ are very small then the relative risk and the odds ratio have similar values (when a probability $p$ is close to $0$, $p/(1-p) \approx p$).  In general, there are two more reasons to **prefer the odds ratio**.
...
**Mutual independence**. all 3 variables at once, that is, outcome, sex and department; 

**Marginal independence**. 2 variables, ignoring the value of the third variable, that is, outcome and sex (we have already done this), outcome and department, sex and department;

**Conditional independence**. 2 variables separately for each value of the third variable, e.g. outcome and sex separately with each department.
