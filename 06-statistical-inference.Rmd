...
*  An **estimate** is the value of an estimator **after** the datahave been observed.  Once we have observed $X_1=x_1, X_2,=x_2, \ldots, X_n=x_n$ the estimate of $\mu$ is
\[ \hat{\mu} = \displaystyle\frac1n\sum_{i=1}^n x_i = \overline{X}. \]
...
```{r echo=FALSE, normalsim, fig.show='hold', fig.cap='The sampling distribution of $\\hat{\\mu}$ and $\\hat{\\sigma}^2$ based on 100,000 independent simulations from a $N(7.22,1.36)$ distribution.  Left: mean.  Right: variance.  The dotted line in the top left plot is the p.d.f. of the $N(7.22,1.36)$ distribution. The sample size is given in the titles of the plots.', out.width = '50%'}
...
Suppose that 
$$X_1, \,\ldots,X_n \,\stackrel{{\rm i.i.d.}}{\sim}\, N(\mu,\sigma^2),$$ 
where $\sigma^2$ is finite.  We define the estimator $\hat{\mu}=\overline{X}=\displaystyle\frac{1}{n}\sum_{i=1}^n\,X_i$ of $\mu$.
...
**Exercise**.  Use the condition in equation \@ref(eq:consistency) to show that $\overline{X}$ is a consistent estimator of $\mu$.

The normality assumption is not necessary here, owing to the weak law of large numbers.

**The weak law of large numbers**.  If $X_1, ..., X_n$ are i.i.d. with $\mbox{E}(X_i)=\mu$ and $\mbox{var}(X_i) = \sigma^2$, for $i=1, ..., n$ then $\overline{X} \overset{p}{\to} \mu$, that is, $\overline{X}$ is a consistent estimator of $\mu$.
...
This is illustrated in Figure \@ref(meanmedian).  Note that this result relies on the assumption that the random sample is from a normal distribution.  If the data are sampled from another distribution then the sample mean may not have a smaller variance that the sample median.
