...
col1 <- c(col1[1:3], "...", col1[(nn - 1):nn])
col2 <- c(rep(1, 3), "...", rep(1, 2))
col3 <- c(rep("1/95", 3), "...", rep("1/95", 2))
col4 <- c(1:3, "...", (nn - 1):nn)
col5 <- c("1/95", "2/95", "3/95", "...", "94/95", "1")
...
(ref:oxfreqecdf-caption) Frequency table of the Oxford birth times, with one observation in each class.
...
oxtabecdf_hux <- oxtabecdf %>% 
  ...
  set_top_border(everywhere, everywhere, brdr(0.1, "solid", "white")) %>%
  set_top_border(c(1:2, 8), everywhere, brdr(1, "solid", "black")) %>%
  set_background_color(everywhere, everywhere, "white") %>%
  set_width(0.8) %>% 
  set_align(everywhere, everywhere, "center") %>% 
  set_number_format(everywhere, 1, 1) %>%
  set_number_format(everywhere, -1, 0) %>%
  set_number_format(8, 3, 0) %>%
  set_number_format(7, 5, 0) %>%
  set_bottom_padding(0) %>%
  set_top_padding(0) %>%
